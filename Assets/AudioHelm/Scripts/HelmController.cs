...
namespace AudioHelm
{
    ...
    public class HelmController : MonoBehaviour, NoteHandler
    {
        ...
        public Dictionary<int, PressedNote> pressedNotesDurations = new Dictionary<int, PressedNote>();
        ...
        // Julian: habe ich hinzugefügt
        void InitJuliansDictionary()
        {
            for (int note = 0; note < Utils.kMidiSize; note++)
            {
                PressedNote pressedNote = new PressedNote();
                pressedNote.duration = 0;
                pressedNotesDurations.Add(note, pressedNote);
            }
        }
        ...
        public IEnumerator WaitNoteOff(int note, float length)
        {
        ...
        // Julian: habe ich hinzugefügt
        private IEnumerator CountNoteOnTime(int note)
        {
            float time = 0;
            while (true)
            {
                time += Time.deltaTime;
                pressedNotesDurations[note].duration = time;
                yield return null;
            }
        }
